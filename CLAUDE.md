...
> `docs/DESIGN_DOC.docx` is NOT auto-generated — it's a committed artifact. After
> editing `docs/DESIGN_DOC.md` (or re-exporting any diagram/screenshot PNG), run
> `npm run docs:docx` to refresh it, or the Word file goes stale.
...
7. Run `npm test` before suggesting a change is done (use `npm test`, NOT
   ...
   enumerates `test/*.test.js`). **277 tests should pass** (as of the 2026-06-04
   review-fix session — day-label continuity + the daily.remaining=0 edge fix,
   /quota preflight retry, monitor poll backoff, opt-in namespace breakdown).
