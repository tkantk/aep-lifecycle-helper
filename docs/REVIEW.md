...
| POST | `/api/jobs/:id/work-orders/:woId/release-absent` | Operator-confirmed resolution for a stuck indeterminate orphan (R7 #1). Body `{ confirmedAbsent: true }` REQUIRED. Releases the held reservation + resets the WO to `planned` for retry. Fail-closed: 409 if the WO has an Adobe ID or an `accepted` reservation; only a `submitting` orphan is eligible |
