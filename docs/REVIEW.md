...
| POST | `/api/jobs/:id/work-orders/:woId/release-absent` | Operator-confirmed resolution for a stuck indeterminate orphan (R7 #1). Body `{ confirmedAbsent: true }` REQUIRED. Releases the held reservation + resets the WO to `planned` for retry, in one transaction (R8 #2). Fail-closed: 409 if the WO has an Adobe ID, an `accepted` reservation, or its POST is still in flight (R8 #1); only a settled `submitting` orphan is eligible |
